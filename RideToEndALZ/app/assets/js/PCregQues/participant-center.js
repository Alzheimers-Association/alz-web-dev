  ...
  adarda.trpc.ui = {
    ...
    getSurveyAnswers: function() {
        ...
        callback: {
          success: function(response) {
            ...
            $.each(surveyResponses, function() {
              ...
              if(surveyKey === 'jersey_size') {
                if(!$.isEmptyObject(responseValue) && (responseValue != '')) {
                  $('.js__jersey-size-field').attr('name', surveyQuestionName);
                  $('.js__jersey-size-answer').html(responseValue);
                  $('.js__jersey-size-field').prev('label').attr('for', surveyQuestionName);

                  var questionAnswers = luminateExtend.utils.ensureArray(surveyResponse.questionAnswer);

                  $.each(questionAnswers, function() {
                    var questionAnswer = this;
                    $('.js__jersey-size-field').append('<option value="' + questionAnswer.value + '">' + 
                                               questionAnswer.label + 
                                             '</option>');
                  });
                  if($('.js__jersey-size-field option[value="' + responseValue + '"]').length > 0) {
                    $('.js__jersey-size-field').val(responseValue);
                  }
                }
              }

              if(surveyKey === 'bike_short_size') {
                if(!$.isEmptyObject(responseValue) && (responseValue != '')) {
                  $('.js__bike-short-size-field').attr('name', surveyQuestionName);
                  $('.js__bike-short-size-answer').html(responseValue);
                  $('.js__bike-short-size-field').prev('label').attr('for', surveyQuestionName);

                  var questionAnswers = luminateExtend.utils.ensureArray(surveyResponse.questionAnswer);
                  $.each(questionAnswers, function() {
                    var questionAnswer = this;
                    $('.js__bike-short-size-field').append('<option value="' + questionAnswer.value + '">' + 
                                               questionAnswer.label + 
                                             '</option>');
                  });
                  if($('.js__bike-short-size-field option[value="' + responseValue + '"]').length > 0) {
                    $('.js__bike-short-size-field').val(responseValue);
                  }
                }
              }

              if(surveyKey === 'medical_condition') {
                if(!$.isEmptyObject(responseValue) && (responseValue != '')) {
                  $('.js__med-note-field').attr('name', surveyQuestionName);
                  $('.js__med-note-answer').html(responseValue);
                  $('.js__med-note-field').prev('label').attr('for', surveyQuestionName);

                  var questionAnswers = luminateExtend.utils.ensureArray(surveyResponse.questionAnswer);
                  $.each(questionAnswers, function() {
                    var questionAnswer = this;
                    $('.js__med-note-field').append('<option value="' + questionAnswer.value + '">' + 
                                               questionAnswer.label + 
                                             '</option>');
                  });
                  if($('.js__med-note-field option[value="' + responseValue + '"]').length > 0) {
                    $('.js__med-note-field').val(responseValue);
                  }
                }
              }
              if(surveyKey === 'medical_condition_details') {
                if(!$.isEmptyObject(responseValue) && (responseValue != '')) {
                  // $('#edit-honorary').val(responseValue);
                  $('.js__med-note-descr-field').val(responseValue).attr('name', surveyQuestionName).closest('label');
                  $('.js__med-note-descr-field').prev('label').attr('for', surveyQuestionName);

                  $('.js__med-note-descr-answer').html(responseValue);
                }
              }

              if(surveyKey === 'OPTIN') {
                optInResponseCount++;
                if(optInResponseCount == 1) {
                  // first opt-in response, build all options
                  // we do this because the API call returns the full question for each checked interest, resulting in duplicates

                  $('.js__interest-optins-answer').html(responseValue);
                
                  var questionAnswers = luminateExtend.utils.ensureArray(surveyResponse.questionAnswer);

                  $.each(questionAnswers, function(index, value) {
                    var questionAnswer = this;
                    $('.js__interest-optins-field').append('<div><input type="checkbox" class="d-inline" name="' + surveyQuestionName + '" id="' + surveyQuestionName + '_' + index + '" value="' + questionAnswer.value + '" data-label="' + questionAnswer.label + '"/><label class="d-inline" for="' + surveyQuestionName + '_' + index + '">' +  questionAnswer.label + '</label></div>');
                  });
                  if($('.js__interest-optins-field option[value="' + responseValue + '"]').length > 0) {
                    $('.js__interest-optins-field').val(responseValue);
                  }
                } else {
                  // not on first response, append answer to existing
                  var answer = $('.js__interest-optins-answer').html();
                  $('.js__interest-optins-answer').html(answer + '<br>' + responseValue);
                }
                // update checked values
                // but first clean up the inconsistent response values (interests include folder names in responseValue but not in questionAnswer.value)
                var interestDelimeter = responseValue.indexOf(' > ') > 0 ? responseValue.indexOf(' > ') + 3 : 0;
                var interestEnd = responseValue.length - 1;
                responseValueTrimmed = responseValue.substr(interestDelimeter, interestEnd);
                console.log('responseValueTrimmed = ', responseValueTrimmed);
                $('input[type*=checkbox][data-label*="' + responseValueTrimmed + '"]').attr('checked', 'checked');
              }
      ...
      adarda.getUser({
        callback: {
          success: function(response) {
            console.log('// GETUSER : called. response is: ', response)
            var getConsResponse = luminateExtend.utils.ensureArray(response.getConsResponse);
            
            /* store the survey responses */
            adarda.trpc.cons.getConsResponse = getConsResponse;
            var optInResponseCount = 0;
            $.each(getConsResponse, function() {
              var consResponse = this;
              $.each(consResponse, function(key, value) {
                console.log('// GETUSER : response key, value: ', key, value)
                format = typeof(value);

                if(key === 'primary_address') {
                    $('.js__street1-field').val(this.street1);
                    $('.js__street1-answer').html(this.street1);
                    $('.js__street2-field').val(this.street2);
                    $('.js__street2-answer').html(this.street2);
                    $('.js__zip-field').val(this.zip);
                    $('.js__zip-answer').html(this.zip);
                    $('.js__city-field').val(this.city);
                    $('.js__city-answer').html(this.city);
                    $('.js__state-field').val(this.state);
                    $('.js__state-answer').html(this.state);
                    $('.js__country-field').val(this.country);
                    $('.js__country-answer').html(this.country);
                }
                if(key === 'birth_date') {
                  ...
                  var month = vals[1];
                  var day = vals[2];
                  $('#rel1_birth_date_MONTH').val(parseInt(month));
                  $('#rel1_birth_date_DAY').val(parseInt(day));
                  $('#rel1_birth_date_YEAR').val(parseInt(year));
                  // // In case want "Jan" displayed instead of "01"
                  // var birthMonth_MMM = $('.js__birthday-month-field option:checked').html();
                  // birthMonth_MMM = (birthMonth_MMM === 'Month') ? '-' : birthMonth_MMM;
                  // $('.js__birthday-month-answer').html(birthMonth_MMM ? birthMonth_MMM : ' - ');
                  $('.js__birthday-month-answer').html(month ? month : ' - ');
                  $('.js__birthday-day-answer').html(day ? day : ' - ');
                  $('.js__birthday-year-answer').html(year ? year : ' - ');
                }
                if(key === 'email') {
                  $('.js__email-field').val(this.primary_address);
                  $('.js__email-answer').html(this.primary_address);
                  $('.js__receive-msg-field').val(this.accepts_email);
                  $('.js__receive-msg-answer').html(this.accepts_email);
                }
                if(key === 'home_phone') {
                  $('.js__phone-field').val(value);
                  $('.js__phone-answer').html(value);
                }
              });
            });
            /* trigger custom event */
            $(document).trigger('consResponses:loaded');
          }, 
          error: handleApiError
        }
      });
      ...
      var emergencyName = adarda.trpc.teamraiserRegistration['tr'+adarda.trpc.frId].emergencyName;
      var emergencyPhone = adarda.trpc.teamraiserRegistration['tr'+adarda.trpc.frId].emergencyPhone;
      $('.js__ride-emergency-contact-field').val(emergencyName);
      $('.js__ride-emergency-contact-answer').html(emergencyName);
      $('.js__emergency-phone-field').val(emergencyPhone);
      $('.js__emergency-phone-answer').html(emergencyPhone);
    },
...
console.log('hide subsequent popover');
 ...
 if(document.cookie.indexOf('sentEmail=true') !== -1) {
  ...
  $('.js__update-reg-form').consProfile();
...
$('.js__cancel-reg-update-btn').on('click', function(e){
  ...
  $('.js__view-less-reg-btn').show();
  $('#update_reg_info .error').hide();
})
$('.js__view-full-reg-btn').on('click', function(e){
  e.preventDefault();
  $('.js__reg-shrink').hide();
  $('.js__reg-full').show();
})
$('.js__view-less-reg-btn').on('click', function(e){
  e.preventDefault();
  $('.js__reg-shrink').show();
  $('.js__reg-full').hide();
})

  $('.js__update-reg-form').validate({
    rules :{
      'primary_address.zip' : {
        // removing since multiple countries
        // zipcodeUS: true 
      },
      'primary_address.street1' : 'required',
      'primary_address.city' : 'required',
      'primary_address.state' : 'required',
      'primary_address.zip' : 'required',
      'primary_address.country' : 'required'
    }
  })
  $('.js__update-reg-form').on('submit', function(e){
    ...
    if(!$elem.valid()){
      ...
      setTimeout(function(){ window.scrollTo(0, $('.error').offset().top)},20);
      return false;
    }
    ...
    var hasJerseySize = $('.js__jersey-size-field').val();
    var hasBikeShortSize = $('.js__bike-short-size-field').val();
    var hasRideRoute = $('.js__ride-route-field').val();
    var hasMedCondition = $('.js__med-note-field').val();
    var hasMedDescription = $('.js__med-note-descr-field').val();
    ...
    var hasInterests_HTMLFormat = '';
    var hasInterests_APIFormat = '';
    var $hasInterestsBuilder = $('.js__interest-optins-field input:checked');
    
    $hasInterestsBuilder.each(function(index, value){
      hasInterests_APIFormat = hasInterests_APIFormat + '&question_key_OPTIN=' + encodeURIComponent($(this).attr('data-label'));
      hasInterests_HTMLFormat = $(this).attr('data-label') + '<br>' + hasInterests_HTMLFormat;
    })

    // listConsFields
    var hasStreet1 = $('#cons-profile-primary-address-street1').val();
    var hasStreet2 = $('#cons-profile-primary-address-street2').val();
    var hasCity = $('#cons-profile-primary-address-city').val();
    var hasState = $('#cons-profile-primary-address-state').val();
    var hasCountry = $('#cons-profile-primary-address-country').val();
    var hasEmail = $('.js__email-field').val();
    var hasPhone = $('.js__phone-field').val();
    var hasEmailOpt = $('.js__receive-msg-field').val();
    var hasBirthday_month = parseInt($('#rel1_birth_date_MONTH').val());
      var hasBirthday_month_twoDigits = ('0' + hasBirthday_month).slice(-2);
    var hasBirthday_day = parseInt($('#rel1_birth_date_DAY').val());
      var hasBirthday_day_twoDigits = ('0' + hasBirthday_day).slice(-2);
    var hasBirthday_year = parseInt($('#rel1_birth_date_YEAR').val());
    var convertedBirthDate_APIFormat = hasBirthday_year + '-' + hasBirthday_month_twoDigits + '-' + hasBirthday_day_twoDigits;
    $('input[name="birth_date"]').val(convertedBirthDate_APIFormat);

    // trReg fields
    var hasEmContact_name = $('.js__ride-emergency-contact-field').val();
    var hasEmContact_phone =$('.js__emergency-phone-field').val();
 
    $(adarda.trpc.teamraiserRegistration['tr'+adarda.trpc.frId].surveyResponses).each(function(){
      var surveyResponse = this, 
      surveyKey = surveyResponse.key || '';
      // console.log('surveyKey: ', surveyKey);
      if(surveyKey !== 'mobile_phone' && surveyKey !== 'ride_honoree' && surveyKey !== 'alz_connection' && surveyKey !== 'tshirt_size' && surveyKey !== 'jersey_size' && surveyKey !== 'bike_short_size' && surveyKey !== 'ride_route' && surveyKey !== 'is_vegetarian' && surveyKey !== 'crew_interests' && surveyKey !== 'volunteer_interests' && surveyKey !== 'medical_condition' && surveyKey !== 'medical_condition_details' && surveyKey !== 'OPTIN')  {
        var responseValue = this.responseValue;
        if(typeof responseValue != 'string' || responseValue === 'User Provided No Response') {
          responseValue = '';
        }
    ...
    adarda.updateSurveyResponses({
      frId: adarda.trpc.frId,
      data: surveyQuestions + (hasMobilePhone ? '&question_key_mobile_phone=' + encodeURIComponent(hasMobilePhone) : '')  + (hasAlzConnection ? '&question_key_alz_connection=' + encodeURIComponent(hasAlzConnection) : '') + (hasHonoree ? '&question_key_ride_honoree=' + encodeURIComponent(hasHonoree) : '') + (hasTshirtSize ? '&question_key_tshirt_size=' + encodeURIComponent(hasTshirtSize) : '') + (hasBikeShortSize ? '&question_key_bike_short_size=' + encodeURIComponent(hasBikeShortSize) : '') + (hasJerseySize ? '&question_key_jersey_size=' + encodeURIComponent(hasJerseySize) : '')  + (hasRideRoute ? '&question_key_ride_route=' + encodeURIComponent(hasRideRoute) : '')  + (hasIsVegetarian  ? '&question_key_is_vegetarian=' + encodeURIComponent(hasIsVegetarian) : '') + (hasVolunteerPosition  ? '&question_key_volunteer_interests=' + encodeURIComponent(hasVolunteerPosition) : '') + (hasCrewPosition  ? '&question_key_crew_interests=' + encodeURIComponent(hasCrewPosition) : '') + (hasMedCondition  ? '&question_key_medical_condition=' + encodeURIComponent(hasMedCondition) : '') + (hasMedDescription  ? '&question_key_medical_condition_details=' + encodeURIComponent(hasMedDescription) : '') + (hasInterests_APIFormat ? hasInterests_APIFormat : ''), 
      callback: {
        success: function(response) {
          if(response.updateSurveyResponsesResponse.success == 'false') {
            /* TODO: updateSurveyResponses error */
            ...
            $('.js__jersey-size-answer').html(hasJerseySize);
            $('.js__bike-short-size-answer').html(hasBikeShortSize);
            $('.js__med-note-answer').html(hasMedCondition);
            $('.js__med-note-descr-answer').html((hasMedDescription === 'User Provided No Response' ? '' : hasMedDescription));
            $('.js__interest-optins-answer').html((hasInterests_HTMLFormat === '' ? 'None Selected' : hasInterests_HTMLFormat));
    ...
    adarda.updateUser({
      data: $('#update_reg_info').find('input, select, textarea').not('[name^="question_"]').serialize(), 
      callback: {
        success: function(response) {
          //$elem.data('conssuccess', 1);
          //showProfileSuccess();
          // TODO: Update the HTML labels for updated elements
          console.log('updateUserResponse', response);
          $('.js__street1-answer').html(hasStreet1);
          $('.js__street2-answer').html(hasStreet2);
          $('.js__city-answer').html(hasCity);
          $('.js__state-answer').html(hasState);
          $('.js__country-answer').html(hasCountry);
          $('.js__email-answer').html(hasEmail);
          $('.js__phone-answer').html(hasPhone);
          $('.js__receive-msg-answer').html(hasEmailOpt);
          // // In case want "Jan" displayed instead of "01"
          // var birthMonth_MMM = $('.js__birthday-month-field option:checked').html();
          // birthMonth_MMM = (birthMonth_MMM === 'Month') ? '-' : birthMonth_MMM;
          // $('.js__birthday-month-answer').html(birthMonth_MMM ? birthMonth_MMM : ' - ');
          $('.js__birthday-month-answer').html(hasBirthday_month ? hasBirthday_month_twoDigits : ' - ');
          $('.js__birthday-day-answer').html(hasBirthday_day ? hasBirthday_day_twoDigits : ' - ');
          $('.js__birthday-year-answer').html(hasBirthday_year ? hasBirthday_year : ' - ');
        }, 
        error: function(response) {
          console.log('updateUserError', response);
          if($.inArray(response.errorResponse.code, ['3', '5', '14']) > -1) {
            window.location = luminateExtend.global.path.secure + 
                              'UserLogin?NEXTURL=' + encodeURIComponent(window.location.href);
          }
    ...
    adarda.trpc.api.teamraiser.updateRegistration({
      data: $('.js__emergency-fields').find('input, select, textarea').serialize(), 
      callback: {
        success: function(response) {
          console.log('emergency fields updated');
          $('.js__ride-emergency-contact-answer').html(hasEmContact_name);
          $('.js__emergency-phone-answer').html(hasEmContact_phone);
        }, 
        error: function(response) {
          console.log('ERROR: emergency fields not updated');
        }
      }
    });
  });
